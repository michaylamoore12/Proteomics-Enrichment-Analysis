...
```{r}
#GO Enrichment
#load data and view (Moore, CM, Treated for 24-hours in 0% FBS)
lung.expr <- read.csv(("Moore_CM_Data_BMP9toCtrl.csv"),stringsAsFactors = FALSE)
head(lung.expr)
```

```{r, warning = FALSE, echo=FALSE}
# Filter unregulated (> 1.1), downregulated ( < 0.7), (,1 takes ID's from column 1)
up.genes <- lung.expr[lung.expr$log2FC > 1.2 & lung.expr$adj.P.Value < 0.05, 1] 
...
go_terms_all_downreg <- ggplot(egobp2[1:20], aes(x=reorder(Description, -pvalue), y=Count, fill=-p.adjust)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    scale_fill_continuous(low="#b3b3b3", high="#AA4499") +
    labs(x = "", y = "", fill = "p.adjust") +
    theme(axis.text=element_text(size=11),
          colour = "black") +
    theme_minimal() +
    ggtitle("Up-Regulated Proteins in Secretome") +
    ylab("Count")
...
go_terms_cardio_downreg <- ggplot(cardio2, aes(x=reorder(Description, -pvalue), y=Count, fill=-p.adjust)) +
    ...
    theme(axis.text=element_text(size=11),
          colour = "black") +
    theme_minimal() +
    ggtitle("Prioritized Cardiovascular Terms (Up-Regulated)") +
    ylab("Count")
...
CC_terms_all_upreg <- ggplot(egobp3[1:7], aes(x=reorder(Description, -pvalue), y=Count, fill=-p.adjust)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    scale_fill_continuous(low="#b3b3b3", high="black") +
    labs(x = "", y = "", fill = "p.adjust") +
...
MF_terms_all_upreg <- ggplot(egobp4[1:11], aes(x=reorder(Description, -pvalue), y=Count, fill=-p.adjust)) +
    geom_bar(stat = "identity") +
    coord_flip() +
